...
import { dashboardProgressPercent } from "../../shared/dashboardProgress";
...
function OverviewViewToggle({
  ...
  function select(view: "services" | "tickets") {
    onChange(view);
    scheduleScrollDashboardGridIntoView();
  }
...
function ServiceDetail({
  ...
  return (
    <section className="detail-panel">
      ...
      >
        {[
          ...
          <button
            ...
            key={tab}
            onClick={() => selectTab(tab as "prs" | "releases" | "branches")}
          >
            {label}
          </button>
        ))}
      ...
      {activeTab === "prs" && (
        <div className="service-operations" role="tabpanel" aria-label="PRs">
                          ...
                          {Boolean(item.pullRequest.participants?.length) && (
                            ...
                            >
                              ...
                              {(item.pullRequest.participants?.length ?? 0) >
                                ...
                                <span>
                                  +
                                  {(item.pullRequest.participants?.length ??
                                    0) - 6}
                                </span>
                              )}
                            </button>
...
export function ReleaseOverview({
  ...
  const [releaseDayOpen, setReleaseDayOpen] = useState(
    () =>
      new URLSearchParams(window.location.search).get("view") === "release-day",
  );
  ...
  const repositoryScope = visibleServices
  ...
  const mergedIssueKeys = new Set(
    visibleServices.flatMap((service) =>
      service.items.filter(isClearedMerge).map((item) => item.issue.key),
    ),
  );
  ...
  async function confirmRemoveTicket(targetVersionId?: string) {
    ...
    const removedIssueKey = pendingRemoveIssueKey;
    ...
    try {
      ...
      setSelectedIssueKey("");
      onIssueRemoved?.(removedIssueKey);
      await onRefresh();
    } catch (reason) {
      setRemoveTicketError(
        reason instanceof Error
          ...
          <div className="dashboard-grid">
            ...
            {selectedOrganizationRepository ? (
              ...
              >
                ...
                <div
                  className="service-detail-tabs skeleton-list"
                  aria-hidden="true"
                >
                ...
                <div
                  className="repository-pr-panel skeleton-list"
                  aria-hidden="true"
                >
        ...
        {!allServicesSelected && loading && !dashboard && (
          <div className="loading-state" role="status" aria-live="polite">
            ...
            <div
              ...
              aria-valuemin={0}
              aria-valuemax={100}
              aria-valuenow={dashboardProgressPercent(dashboardProgress)}
            >
              <span
                style={{
                  width: `${dashboardProgressPercent(dashboardProgress)}%`,
                }}
              />
            </div>
          </div>
                ...
                <div>
                  ...
                  <div className="overview-bulk-merge-actions">
                    ...
                    <button
                      ...
                      disabled={
                        loading || bulkMerging || visibleServices.length === 0
                      }
                    >
              ...
              <div className="dashboard-grid">
                <section className="services-panel">
                  ...
                  <div className="service-filters" aria-label="Filter services">
                    ...
                    >
                      Back-merges{" "}
                      <span>{risksLoading ? "…" : backMergeServiceCount}</span>
                    </button>
                    <button
